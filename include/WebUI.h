...
<body>
        ...
        <div class="card">
            ...
            <form id="adminForm" onsubmit="saveAdmin(event)" style="margin-bottom: 20px;">
                <div class="form-group">
                    ...
                    <input type="text" id="admin_user" name="admin_user" placeholder="VD: admin" required>
                </div>
                <div class="form-group">
                    <label for="admin_pass">Mật khẩu Admin mới</label>
                    <input type="password" id="admin_pass" name="admin_pass" placeholder="Nhập mật khẩu mới" required>
                </div>
    ...
    <script>
        ...
        fetch('/get_config').then(r => r.json()).then(data => {
            ...
            for(let i=0; i<7; i++) {
                if(data.days && (data.days & (1 << i))) {
                    document.querySelector(`input[name="day_${i}"]`).checked = true;
                }
            }
        });
